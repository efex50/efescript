use crate::{as_be_bytes, runtime::data_funs::get_inner_ptr};
use num_traits::FromPrimitive;

use crate::{ops::OpCodes, runtime::{parse_register_type_to_op, Instuction, OperandType, Operands, SimpleOperands}};
...
impl ProgramRuntime{
    fn get_operand(&mut self) -> Option<Operands>{
        ...
        let l = self.program.read(self.counter, 1).try_into().unwrap();
        ...
        let op = match l {
            ...
            SimpleOperands::Static => {
                ...
                let len = self.program.read(self.counter, 1).try_into().unwrap();
                ...
                if len == 0 {
                    ...
                    self.counter +=1;
                    let a = self.program.read(self.counter, len as usize);
            ...
            SimpleOperands::Ptr => {
                let innerptr = get_inner_ptr(&mut self.counter,&mut self.program);
    ...
    fn get_instruction(&mut self) -> Option<Instuction>{
        let op = self.program.read(self.counter, 1).try_into().unwrap();
